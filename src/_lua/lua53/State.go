...
extern void lua53_luainit(lua_State*);
extern void lua53_pushclosure(lua_State*, int);
extern int  lua53_yield(lua_State*, int);
extern void lua53_rawseti(lua_State*, int, int);
extern void lua53_rawgeti(lua_State*, int, int);
extern int  lua53_pcall(lua_State*, int, int, int);
extern void lua53_call(lua_State*, int, int);
*/
import "C"
...
func docallback(stateindex C.float, funcindex C.float) int {
    // pull our *State value and Function value from lua.GlobalRegistry
    stateval, stateerr := lua.GlobalRegistry.GetValue(int(stateindex)); if stateerr != nil {
        panic(stateerr.Error())
    }
    fnval, fnvalerr := lua.GlobalRegistry.GetValue(int(funcindex)); if fnvalerr != nil {
	...
	fn, ok := fnval.(lua.Function); if !ok {
        panic("Error Casting Function Interface")
    }
...
func (this *State) Init() {
    this.gvindex = lua.GlobalRegistry.AddValue(this)
    C.lua53_luainit(this.luastate)
}
...
func (this *State) Yield(nresults int) int {
	return int(C.lua53_yield(this.luastate, C.int(nresults)))
}
...
// the stack to.
func (this *State) Xmove(from lua.State, n int) {
	C.lua_xmove(from.(*State).luastate, this.luastate, C.int(n))
}
...
func (this *State) Tonumber(index int) float64 {
	return float64(C.lua_tonumberx(this.luastate, C.int(index), nil))
}
...
func (this *State) Tointeger(index int) int {
	return int(C.lua_tointegerx(this.luastate, C.int(index), nil))
}
...
func (this *State) Setglobal(name string) {
    ...
    C.lua_setglobal(this.luastate, ck)
}
...
func (this *State) Resume(narg int) (yield bool, e error) {
	switch r := int(C.lua_resume(this.luastate, nil, C.int(narg))); {
	case r == LUA_YIELD:
		return true, nil
	case r == LUA_OK:
		return false, nil
...
func (this *State) Replace(index int) {
    C.lua_copy(this.luastate, C.int(-1), C.int(index))
    this.Pop(1)
}
...
func (this *State) Remove(index int) {
    C.lua_rotate(this.luastate, C.int(index), -1)
    this.Pop(1)
}

// Sets the Go function fn as the new value of global name.
func (this *State) Register(fn lua.Function, name string) {
...
func (this *State) Rawseti(index, n int) {
	C.lua53_rawseti(this.luastate, C.int(index), C.int(n))
}
...
func (this *State) Rawgeti(index, n int) {
	C.lua53_rawgeti(this.luastate, C.int(index), C.int(n))
}
...
func (this *State) Pushclosure(fn lua.Function, n int) {
	...
	C.lua53_pushclosure(this.luastate, C.int(n + 2))
}
...
// to receive its parameters and return its results (see Function).
func (this *State) Pushfunction(fn lua.Function) {
    this.Pushclosure(fn, 0)
}
...
func (this *State) Pushmodule(name string, fn lua.Function) {    
...
// Pushmetatable pushes a Metatable struct to the stack and conducts the appropriate mapping
// of metatable keys to Functions.
// 
// you must still use Setmetatable after this function returns to assign your metatable to some 
// other table on the stack.
func (this *State) Pushmetatable(mt *lua.Metatable) {
    this.Newtable()    
...
func (this *State) Pcall(nargs, nresults, errfunc int) error {
    ...
    r := int(C.lua53_pcall(this.luastate, C.int(nargs), C.int(nresults), C.int(errfunc)))        
...
func (this *State) Newthread() lua.State {
    if !this.Checkstack(1) {
        panic("STATE: unable to grow lua_state stack")
    }
...
func (this *State) Insert(index int) {
	C.lua_rotate(this.luastate, C.int(index), C.int(1))
}
...
func (this *State) Call(nargs, nresults int) {
	C.lua53_call(this.luastate, C.int(nargs), C.int(nresults))
}
...
func (this *State) Loadfile(filename string) error {
    ...
    return this.geterror(int(C.luaL_loadfilex(this.luastate, cs, nil)))
}
